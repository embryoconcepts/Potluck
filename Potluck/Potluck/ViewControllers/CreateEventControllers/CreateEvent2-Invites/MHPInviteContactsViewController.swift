...
protocol ContactsSelectedDelegate: AnyObject {
...
class MHPInviteContactsViewController: UIViewController {
        ...
        DispatchQueue.main.async {
            tempInvites = self.allContacts
                ...
                .filter { invite -> Bool in
                    if !self.pendingInvites!.isEmpty, self.pendingInvites!.contains(invite) {
                        return false
                    } else {
                        return true
                    }
    ...
    func getContacts() {
        ...
        if CNContactStore.authorizationStatus(for: .contacts) == .notDetermined {
            store.requestAccess(for: .contacts, completionHandler: { (authorized: Bool, _: Error?) -> Void in
                if authorized {
                    ...
                    DispatchQueue.main.async { [unowned self] in
                        ...
                        let settingsAction = UIAlertAction(title: "Open Settings", style: .default, handler: { _ in
                            UIApplication.shared.open(URL(string: UIApplication.openSettingsURLString)!, options: .init(), completionHandler: nil)
                        })
    ...
    func updateContactsWithPreviouslySelected() {
        for invite in self.pendingInvites! {
            for contact in allContacts where invite.contactID == contact.identifier {
...
extension MHPInviteContactsViewController: UITableViewDelegate, UITableViewDataSource {
        ...
        DispatchQueue.main.async { [unowned self] in
            if contact.emails.count == 1 {
                ...
                for email in contact.emails {
                    let button = UIAlertAction(title: email, style: .default) { _ in
                        cell.accessoryType = .checkmark
    ...
    fileprivate func contactDeselected(cell: MHPContactsCell, contact: CNContact, indexPath: IndexPath) {
        ...
        for (i, invite) in self.pendingInvites!.enumerated().reversed() where invite.contactID == contact.identifier {
