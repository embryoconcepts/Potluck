...
protocol CreateEvent3DataDelegate: AnyObject {
...
class MHPCreateEvent3ItemsViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if event?.requestedItems == nil || event?.requestedItems != nil {
            setupSuggestedItems()
        } else {
...
extension MHPCreateEvent3ItemsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return cellItem.cellForTableView(tableView: tblView, atIndexPath: indexPath) as? MHPRequestedItemsCell ?? MHPRequestedItemsCell()
    }
...
extension MHPCreateEvent3ItemsViewController: UIPopoverPresentationControllerDelegate {
    ...
    fileprivate func showInfoPopup() {
        guard let infoPopover = storyboard?.instantiateViewController(withIdentifier: "MHPSuggestedItemInfoPopoverViewController") as? MHPSuggestedItemInfoPopoverViewController else { return }
...
extension MHPCreateEvent3ItemsViewController: ModifyItemPopoverDelegate, CancelAddingNewItemDelegate {
    func addModifyItemPopover(at indexPath: IndexPath, for isNewItem: Bool) {
        guard let modifyItemPopover = storyboard?.instantiateViewController(withIdentifier: "MHPModifyItemPopoverViewController") as? MHPModifyItemPopoverViewController else { return }
